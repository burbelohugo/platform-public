...
describe('add_user', () => {
  ...
  it('Initializes campfire team fields', async () => {
    const response = await adder.run(user_with_team);
    const newUser = JSON.parse(response.body);
    const expectedTeam = JSON.parse(user_with_team.body).campfire_team;

    expect(newUser.campfire_team).toEqual(expectedTeam);
    expect(newUser.points).toEqual(0);
  });

  it('Assigns a team if none is given', async () => {
    const response = await adder.run(user_with_no_team);
    const newUser = JSON.parse(response.body);

    expect(newUser.campfire_team).toEqual(expect.stringMatching(/^red|blue|green$/));
  });
